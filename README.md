SpareRoom / SpareB&B – Sprint 2 (UoR SD3)
...
Host
Holds host details and (optionally) references to their accommodations.

Guest
Represents a user who can book accommodations.
...
Booking
Connects one Guest to one Accommodation over a date range:
...
BookingSystem
Owns the collections and provides the main API:
...
Guest and accommodation must exist
If either ID is unknown, the method logs/throws an error and does not create a booking.

Date range must be valid
startDate must be before endDate.

No overlapping bookings
For every existing booking b of the same accommodation:
...
If all checks pass, the new Booking is created and stored in the bookings list.
...
system.addGuest(new Guest(1, "Alice"));
system.addHost(new Host(1, "Bob"));
...
OOP Design Notes (for your presentation)

You can say something like this out loud:

“This project is built with object-oriented programming.
Each real-world concept is a class: Host, Guest, Accommodation, Booking, and a BookingSystem that coordinates them.
...
Simple room booking prototype for Software Development 3 (University of Roehampton), Sprint 2.
Focus: OOP design, basic booking workflow, and validation against double-booking.
